view: visit_facts_dt {
  derived_table: {
    sql: SELECT
        concat(transaction_present.id_consumer,(DATE(from_unixtime(transaction_present.date_redeemed) )))   AS `transaction_present.visit`,
  ...
  dimension: visit {
    primary_key: yes
    type: string
    sql: ${TABLE}.`transaction_present.visit` ;;
  }
